...
단위 테스트는 테스트 피라미드의 토대가 된다. 일반적으로 하나의 클래스를 인스턴스화 하고 해당 클래스의 인터페이스를
통해 기능들을 테스트한다. 테스트하는 클래스가 다른 클래스에 의존하고 있다면 의존되는 클래스는 인스턴스화 하지 않고

테스트하는 동안 목(mock) 클래스로 대체한다.
```
```
* 통합 테스트
...
인커밍 포트 인터페이스가 구현되는 유스케이스를 테스트하기 SendMoneyService 유스케이스는 커멘드 모델의 금액 한도를 체크하고 
(100 만원), 차변 계좌와 대변 계좌를 로드해온다.

차변 계좌와 대변 계좌의 유무를 체크하고, 대변 계좌에 락을 걸고 차변계좌에서 대변 계좌로 한도 금액 만큼 전송한다 그리고 각각의 
계좌 활동 내역을 업데이트한 후 락을 해제.
...
테스트 성공 후 소프트웨어를 배포해도 될 만큼 신뢰할 수 있으면 된다. 자주 배포하게 된다면 자주 테스트하게 되기 때문에

더 신뢰할 수 있다.

첫 배포에는 신뢰의 도약이 필요하다. 프로덕션의 버그를 수정하고 이로부터 배우는 것을 우선순위로 삼자.
...
테스트는 기능 개발 중에 이루어 져야 한다. 새로운 필드를 추가할 때마다 테스트를 고치는 데 시간을 오래 소비한다면

테스트의 구조가 잘못된 것이다. 

리팩토링 할 때마다 테스트 코드의 변경을 수반한다면 테스트로서의 가치를 잃게 된다.

#### + 육각형 아키텍처에서 테스트를 사용하여 유지보수성을 높이기

육각형 아키텍처는 핵심 코어(도메인, 포트) 와 어댑터 구현체로 분리되어 있다.

도메인과 유스케이스는 단위 테스트를, 어댑터는 통합 테스트를 처리하는 명확한 전략을 세울 수 있다.

이때 입출력 포트는 뚜렷한 모킹 지점으로 입출력 포트 인터페이스가 핵심 메서드만 담고 적은 메서드를 제공할 수록

테스트에서 어떤 포트를 모킹할지 뚜렷하게 구분할 수 있다.

테스트에서 커버리지를 위한 모킹 포트를 만들 때 어떤 것을 모킹해야할지 애매하다면 아키텍처의 유지보수, 응집성에 대한 경고 신호가 된다.
